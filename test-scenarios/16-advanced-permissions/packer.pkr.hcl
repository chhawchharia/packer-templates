...
provisioner "shell" {
  inline = [
    ...
    "sudo cp -r /tmp/secured-scripts/* ${var.secure_dir}/bin/ 2>/dev/null || echo 'No secured scripts to copy'",
    "",
...
provisioner "shell" {
  inline = [
    ...
    "sudo cp -r /tmp/elevated-scripts/* /usr/local/sbin/harness/ 2>/dev/null || echo 'No elevated scripts to copy'",
    "",
...
provisioner "shell" {
  inline = [
    ...
    "echo '=============================================='",
    "",
    "# Copy restricted configs to secure location",
    "sudo cp -r /tmp/restricted-scripts/* ${var.secure_dir}/config/ 2>/dev/null || echo 'No restricted configs to copy'",
    "",
    ...
    "ls -la ${var.secure_dir}/config/ || echo '(empty)'"
  ]
}
